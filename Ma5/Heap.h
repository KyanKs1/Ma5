...
private:
	...
	/********************* Start Microassignment zone ********************/
		 ...
		 */
	//text book implmentation but with minor changes
	 ...
	 */
	//mirror but uses while loop instead of forloop. Shorter becuase it only checks parent instead of two children
	void percolateUp(long unsigned int index)
	{

		T temp = std::move(_items[index]);
		while (index > 1 && temp < _items[floor(index / 2)]) {
			_items[index] = std::move(_items[index / 2]);
			index /= 2;
		}
